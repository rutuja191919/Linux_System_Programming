...
int main()
{
    ...
    ptr = dlopen("./Additionlibrary.so",RTLD_LAZY);   //runtime load lazy : if that so file is already on RAM then use that its lazy
    ...
    ptr = dlopen("./Substractionlibrary.so",RTLD_LAZY);   //runtime load lazy : if that so file is already on RAM then use that its lazy
